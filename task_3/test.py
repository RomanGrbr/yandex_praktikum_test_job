...
class TestTask:
    ...
    def test_long_heavy(self):
        ...
        assert precode.long_heavy(4) == author.long_heavy(4), (
            "Не изменяйте функцию `long_heavy()`"
        )
    ...
    def test_cache_args(self):
        ...
        # long_heavy = author.long_heavy.__closure__[0].cell_contents
        cache_args = precode.cache_args
        result = []
        for i in (1, 1, 2, 2, 2):
            result.append(time_check(cache_args(foo))(i))
        assert result == [1.0, 0.0, 1.0, 0.0, 0.0], (
            "Функция плохая"
        )
